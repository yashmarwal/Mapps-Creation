    ...
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            data-lenis-prevent
            className="fixed inset-0 z-[200] flex items-center justify-center p-0 sm:p-4 md:p-6 bg-black/85 backdrop-blur-2xl overflow-hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              className="relative w-full h-[100dvh] sm:h-auto sm:max-h-[85vh] sm:max-w-3xl bg-[#0A1628]/98 backdrop-blur-3xl border-0 sm:border border-[var(--gold)]/40 rounded-none sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden"
            ...
            >
              ...
              <div className="relative p-3.5 sm:p-6 pb-3 border-b border-border/60 bg-slate-950/80 shrink-0">
                ...
                <div className="relative flex items-center bg-slate-900/90 border border-primary/30 rounded-xl sm:rounded-2xl px-3.5 py-2.5 sm:py-3 shadow-inner">
                  ...
                  {query ? (
                    <button
                      onClick={() => setQuery("")}
                      className="text-muted-foreground hover:text-foreground text-[11px] font-medium px-2 py-0.5 rounded bg-slate-800 mr-1.5 cursor-pointer shrink-0"
                    >
                      Clear
                    </button>
                  ) : null}
              ...
              >
                <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground shrink-0 mr-1 flex items-center gap-1">
              ...
              {!query && popularSearches.length > 0 && (
                <div
                  ...
                  style={{ touchAction: "pan-x", scrollbarWidth: "none" }}
                >
                  <span className="text-[10px] font-medium uppercase tracking-wider text-amber-400 flex items-center gap-1 mr-1 shrink-0">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="relative h-14 w-14 sm:h-16 sm:w-16 shrink-0 overflow-hidden rounded-xl border border-border/80 bg-slate-950 cursor-pointer"
                          >
                            <img
                              src={product.image}
                              alt={product.name}
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span className="text-primary font-semibold text-xs sm:text-sm shrink-0">
                                ₹{product.price}/{product.unit}
                              </span>
                            </div>

                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                              <span className="text-[9px] uppercase font-medium tracking-wider px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30">
                                {product.category}
                              </span>
                        ...
                        <div className="grid grid-cols-2 gap-1.5 pt-1 border-t border-slate-800/60">
                          <button
                            ...
                            className={`w-full py-1.5 px-2 rounded-lg text-[10px] font-semibold uppercase tracking-wide flex items-center justify-center gap-1 transition-all cursor-pointer active:scale-95 ${
                              isAdded
              ...
              <div className="px-3.5 sm:px-6 py-3 bg-slate-950 border-t border-border/40 flex items-center justify-between text-xs text-muted-foreground shrink-0">
                <span>
                  Found <strong className="text-foreground">{filteredProducts.length}</strong>{" "}
                  fabrics
                </span>
                <button
                  onClick={onClose}
      ...
      <ProductPreviewModal item={previewProduct} onClose={() => setPreviewProduct(null)} />
    </>
  );
}
